---
title: "Connect your own domain to the AI TwinBrain Newsroom"
sidebarTitle: "Custom Domain"
description: "Connect your own domain to the AI TwinBrain Newsroom by adding a CNAME DNS record, takes about 5 minutes to configure, then up to 2 hours to propagate."
---

Connect your own domain to your AI TwinBrain Newsroom so journalists and readers visit a branded URL like `news.yourcompany.com`, not a AI TwinBrain subdomain. This takes about 5 minutes to configure, then 30 minutes to 2 hours for DNS propagation.
...
<Steps>
  <Step title="Open Domain settings in AI TwinBrain">
    Go to **Newsroom Settings > Domain** in your AI TwinBrain dashboard.
  </Step>
  ...
  <Step title="View your DNS configuration details">
    Click **View Instructions**. AI TwinBrain displays the exact DNS values you need to enter with your domain provider.
  </Step>
  <Step title="Open your domain provider in a new tab">
    Log in to your domain registrar and navigate to **DNS Settings** or **DNS Management**. Keep your AI TwinBrain tab open so you can reference the configuration values.
  </Step>
  ...
  <Step title="Set the Value field">
    Enter `target.aitwinbrain.ai` in the **Value** (or **Points to**) field. Copy this exactly, no trailing spaces or extra characters.
  </Step>
  ...
  <Step title="Initiate the connection in AI TwinBrain">
    Return to AI TwinBrain and click **Connect Domain** to start the verification process.
  </Step>
  <Step title="Wait for DNS propagation">
    DNS changes typically propagate within 30 minutes to 2 hours. AI TwinBrain will display a success confirmation when your domain is connected and verified.
  </Step>
</Steps>

<Warning>
  Always use a subdomain (e.g., `news.yourcompany.com`), not your root domain (`yourcompany.com`). AI TwinBrain requires a subdomain for the CNAME configuration, connecting a root domain will not work.
</Warning>

## Troubleshooting

**Connection is taking longer than 2 hours**
DNS propagation can take up to 48 hours with some providers, this is normal and outside of AI TwinBrain's control. Check again after a few hours before contacting support.

**Seeing a connection error**
Double-check two things: the subdomain you entered in AI TwinBrain includes the full address (e.g., `news.yourcompany.com`, not just `news`), and the CNAME value is exactly `target.aitwinbrain.ai`. Even a single character difference will prevent the connection.
...
If you're still having trouble, contact the AI TwinBrain support team at [support@aitwinbrain.ai](mailto:support@aitwinbrain.ai).
